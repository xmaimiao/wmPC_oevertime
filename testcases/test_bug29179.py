from common.contants import basepage_dir, test_bug29179_dir
...
def get_data(option):
    ...
    获取yaml测试数据
    '''
    with open(test_bug29179_dir, encoding="utf-8") as f:
...
class Test_Bug29179:
    ...
    def test_the_fir_for_approved_HR(self):
        ...
        第一行数据審批通過，无tips
        '''
        self.test_the_fir_for_approved_HR()

    def test_the_fir_for_not_approved_HR(self):
        '''
        第一行数据：審批不通過
        '''
        self.the_fir_for_not_approved_HR()
    ...
    def test_HR_reminder_and_not_approved_not_tips(self, data):
        '''
        验证HR提醒补充资料且不批准，非輪班人員下班時間加班，輪班人員全時段加班（无tips）
        '''
        self.logout_fir_and_login(data["applicant"])
        self.overtime_application_not_tips(data["UserTel"],data["startDate"],data["starttime"],
                                           data["endtime"],data["remark"],data["sleeps"],data["IDcard"])
